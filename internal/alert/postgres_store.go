...
func (s *PostgresStore) Get(org string) (Rule, error) {
	var value Rule
	err := s.db.QueryRowContext(context.Background(), `SELECT organization_id,threshold,enabled,updated_at FROM stock_alert_rules WHERE organization_id=$1`, org).
...
func (s *PostgresStore) ListEnabled() []Rule {
	rows, err := s.db.QueryContext(context.Background(), `SELECT organization_id,threshold,enabled,updated_at FROM stock_alert_rules WHERE enabled=true ORDER BY organization_id`)
	if err != nil {
		return []Rule{}
	}
	defer rows.Close()

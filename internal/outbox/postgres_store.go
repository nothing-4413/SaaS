...
func (s *PostgresStore) transitionError(id string) error {
	var status Status
	e := s.db.QueryRowContext(context.Background(), `SELECT status FROM outbox_events WHERE id=$1`, id).Scan(&status)
...
func (s *PostgresStore) MarkPublished(id string, at time.Time) error {
	result, e := s.db.ExecContext(context.Background(), `UPDATE outbox_events SET status='published',claimed_until=NULL,published_at=$2,last_error='' WHERE id=$1 AND status='processing'`, id, at)
	if e != nil {
		return e
	}
...
func (s *PostgresStore) MarkFailed(id string, next time.Time, reason string, terminal bool) error {
	...
	result, e := s.db.ExecContext(context.Background(), `UPDATE outbox_events SET status=$2,claimed_until=NULL,next_attempt_at=$3,last_error=$4 WHERE id=$1 AND status='processing'`, id, status, next, reason)
	if e != nil {
		return e
	}
	n, _ := result.RowsAffected()
...
func (s *PostgresStore) Get(id string) (Event, error) {
	v, e := scanEvent(s.db.QueryRowContext(context.Background(), `SELECT `+eventColumns+` FROM outbox_events WHERE id=$1`, id))
	if errors.Is(e, sql.ErrNoRows) {
